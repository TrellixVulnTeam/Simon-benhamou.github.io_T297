...
router.put("/update/:petID",upload.single('image'),async (req,res) => {
      ...
      try{
            ...
            await Pets.updateOne({id : petID},{$set: {type: req.body.type}})
            await Pets.updateOne({id : petID},{$set: {name: req.body.name}})
            await Pets.updateOne({id : petID},{$set: {status: req.body.status}})
            await Pets.updateOne({id : petID},{$set: {color: req.body.color}})
            await Pets.updateOne({id : petID},{$set: {weight: req.body.weight}})
            await Pets.updateOne({id : petID},{$set: {height: req.body.height}})
            await Pets.updateOne({id : petID},{$set: {bio: req.body.bio}})
            await Pets.updateOne({id : petID},{$set: {hypoallergenic: req.body.hypoallergenic}})
            await Pets.updateOne({id : petID},{$set: {dietaryRestriction: req.body.dietaryRestriction}})
            await Pets.updateOne({id : petID},{$set: {breedOfAnimal: req.body.breedOfAnimal}})
            await Pets.updateOne({id : petID},{$set: {fosterBy: req.body.fosterBy}})
            await Pets.updateOne({id : petID},{$set: {adoptedBy: req.body.adoptedBy}})
...
router.put("/foster/:petID/:userID", async (req, res) => {
    const {petID,userID} = req.params
     await Pets.updateOne({id : petID}, {$set : {fosterBy: userID}})
     await Pets.updateOne({id : petID}, {$set : {status:"Fostered"}})
     res.send("updated")
})

router.delete("/foster/:petID", async (req, res) => {
    const {petID} = req.params
    await Pets.updateOne({id : petID}, {$set : {fosterBy: "none"}})
    await Pets.updateOne({id : petID}, {$set : {status: "Available"}})
     res.send("deleted")
...
router.put("/adopted/:petID/:userID", async (req, res) => {
    ...
    try{
     ...
     await Pets.updateOne({id : petID}, {$set : {adoptedBy: userID}})
     await Pets.updateOne({id : petID}, {$set : {status:"Adopted"}})
     res.send("Adopted")
...
router.delete("/adopted/:petID", async (req, res) => {
    ...
    await Pets.updateOne({id : petID}, {$set : {adoptedBy: "none"}})
    await Pets.updateOne({id : petID}, {$set : {status: "Available"}})
